...
def build_vocab(data_file, nlp, tokenizer, max_size):
...
def load_datasets(data_file, vocab, val_size=0.1, test_size=0.1):
    x = []
    y = []
    ...
    for item in tqdm(ds, desc="Loading data  "):
        x.append([vocab.get_index(t) for t in item["x"]])
        y.append(int(item["y"]))
...
def run_training(args):
    ...
    train_set, val_set, _ = load_datasets(args.data, vocab)
    ...
    model = CNNModel(vocab, embedding_dim=300, num_filters=100, filter_sizes=[2,3,4,5])
